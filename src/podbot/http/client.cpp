...
http_client::http_client(asio::io_service &io)
        : service_(io), hps_(HeaderParserState::HeaderStart), ssl_ctx_(asio::ssl::context::sslv23), resolver_(service_),
          socket_(service_), ssl_socket_(service_, ssl_ctx_) 
{
...
Response http_client::Req(Request *req) {
    ...
    size_t bytes_parsed = http_parser_execute(&parser, &parser_settings_, sstr.str().c_str(),
                                              strlen(sstr.str().c_str()));
    res.set_status_code(parser.status_code);
